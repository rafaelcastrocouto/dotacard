...
var path = require('path'),
  ...
  sendRaw = function(response, content, type, status){
...
//      'Cache-Control': 'public, max-age=31536000',
  ...
  sendZip = function(response, content, type, status){
    response.writeHead(status || 200, {
...
//      'Cache-Control': 'public, max-age=31536000',
  ...
  error = function(response){
    readCache(response, '404.html', 'text/html', 0,  404);
  },

  readCache = function(response, file, type, zip, status){
    if(!zip && cache[file]) {
      sendRaw(response, cache[file], type);
    } else if(zip && cache[file+'.zip']) {
      sendZip(response, cache[file+'.zip'], type);
    } else {
      fs.readFile(file, function (err, content) {
        ...
        if(err) {
          ...
          if (!zip) {
            sendRaw(response, content, type, status);
          } else {
            zlib.gzip(new Buffer(content), function(err, data){
              if(err) {
                ...
                cache[file+'.zip'] = data;
                sendZip(response, data, type, status);
              }
            });
          }
        }
