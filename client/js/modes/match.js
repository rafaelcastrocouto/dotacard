game.match = {
  build: function () {
    if (!game.match.builded) {
      game.match.builded = true;
      game.mode = 'match';
  ...
  searching: function () {
    game.db({ 'get': game.id }, function (found) {
      if (found.challenger) {
        ...
        game.match.battle(found.challenger, 'challenger');
      } else {
        ...
        } else { game.timeout(1000, game.match.searching); }
      }
    });
  },
  found: function () {
  ...
  battle: function (enemy, challenge) {
    ...
    game.timeout(1000, game.match.pickCount);
  },
  pick: function () {
  ...
  pickCount: function () {
    ...
    if (game.states.choose.count < 0) {
      ...
      game.match.fillDeck();
    } else { game.timeout(1000, game.match.pickCount); }
  },
  fillDeck: function () {
    $('.slot').each(function () {
      var slot = $(this), card;
    ...
    game.db({ 'get': game.id }, function (found) {
      if (found.challengerDeck) {
        ...
        if (game.tries > game.connectionLimit) {
          game.reset();
        } else { game.timeout(1000, game.match.getChallengerDeck); }
      }
    });
  },
  getChallengedDeck: function () {
    ...
    game.db({ 'get': game.id }, function (found) {
      if (found.challengedDeck) {
        ...
        if (game.tries > game.connectionLimit) {
          game.reset();
        } else { game.timeout(1000, game.match.getChallengedDeck); }
      }
    });
  },
  setTable: function () {
      ...
      game.player.heroesDeck = game.deck.build({
        ...
        cb: function (deck) {
          ...
          $.each(deck.data('cards'), function (i, card) {
            ...
            card.addClass('player hero').data('side', 'player').onClickEvent(game.card.select);
      ...
      game.enemy.heroesDeck = game.deck.build({
        ...
        cb: function (deck) {
          ...
          $.each(deck.data('cards'), function (i, card) {
            ...
            card.addClass('enemy hero').data('side', 'enemy').onClickEvent(game.card.select);
    ...
    game.player.skills.deck = game.deck.build({
      ...
      cb: function (deck) {
        ...
        $.each(deck.data('cards'), function (i, skill) {
          skill.addClass('player skill').data('side', 'player').onClickEvent(game.card.select);
  ...
  getData: function () {
    ...
    game.db({ 'get': game.id }, function (data) {
      if (data[game.enemy.type + 'Turn'] === game.enemy.turn) {
        ...
        if (game.tries > game.connectionLimit) {
          game.reset();
        } else { game.timeout(1000, game.match.getData); }
      }
    });
  },
  win: function () {
